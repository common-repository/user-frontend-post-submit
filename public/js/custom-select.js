...
{	
	...
	{	  
		...
		document.onkeypress = function(e)
		{
			...
			if ($(".custom-select ul." + config.focusedClass).length > 0 && config.catchKeypress)
			{
				config.catchKeypress = false;
				...
				$(".custom-select ul." + config.focusedClass).each(function()
				{
					...
					var flag = false;
					customSelectDropdown.find("li").each(function()
					{
						if (typedChar.toLowerCase() == $(this).text().charAt(0).toLowerCase() && !flag)
						{
							flag = true;
							...
							{
								customSelectDropdown.find("li." + config.selectedClass).removeClass(config.selectedClass);
								nextElement.addClass(config.selectedClass);
								
								if (customSelectDropdown.find("li." + config.selectedClass).is(":visible"))
								{
									var scrollingOffset = nextElement.offset().top - customSelectDropdown.offset().top;
									var scrollingOffsetTotal = (scrollingOffset - customSelectDropdown.height() > -customSelectDropdown.find("li." + config.selectedClass).height()*4)?scrollingOffset - customSelectDropdown.height() + customSelectDropdown.find("li." + config.selectedClass).height()*4:scrollingOffset - customSelectDropdown.height() + $(this).height()*4;
									customSelectDropdown.find("li." + config.selectedClass).parent().scrollTop(scrollingOffsetTotal + customSelectDropdown.find("li." + config.selectedClass).parent().scrollTop());
								}
								...
								if (typedChar.toLowerCase() == String.fromCharCode(e.which).toLowerCase()) customSelect.next().change();
							}
						}
					});
				});
		...
		{
			//keypress flag for non-ascii chars
			config.catchKeypress = true;
			...
			$(".custom-select ul." + config.focusedClass).each(function()
			{
				...
				if ((e.which >= 65 && e.which <= 90) || (e.which >= 97 && e.which <= 122))
				{
					...
					customSelectDropdown.find("li").each(function()
					{
						if (String.fromCharCode(e.which).toLowerCase() == $(this).text().charAt(0).toLowerCase() && !flag)
						{
							flag = true;
		...
		return this.each(function()
		{
			...
			customSelect.click(function(event)
			{
				...
				else
				{
					...
					customSelect.find('.custom-select-current').focus();
				}
